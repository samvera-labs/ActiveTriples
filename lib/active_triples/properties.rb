  ...
  module Properties
    ...
    module ClassMethods
      ...
      def property(name, opts = {}, &block)
        if protected_property_name? name
          raise ArgumentError, "#{name} is a keyword and not an acceptable property name."
        end
      ...
      def protected_property_name?(name)
        reject = instance_methods.map! { |s| s.to_s.gsub(/=$/, '').to_sym }
        reject -= properties.keys.map(&:to_sym)
        reject.include? name
      end
